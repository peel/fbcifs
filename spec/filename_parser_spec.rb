require 'rspec'
require_relative '../lib/fbcifs/filename_parser'

describe Fbcifs::FilenameParser, '#is_an_encryption' do
  it "checks if file is encrypted version of given filename" do
    ...
    parser = Fbcifs::FilenameParser.new
    expect(files.map{|file| parser.is_an_encryption(file)}).to eq expected
  end
end

describe Fbcifs::FilenameParser, '#encrypted_filename_variants' do
...
describe Fbcifs::FilenameParser, '#file_and_encrypted' do
  it "returns list of all variations of a filename" do
...
describe Fbcifs::FilenameParser, '#filter_out_file_and_encrypted' do
